...
static void wait_go_out(Ghost* ghost);
...
void start_go_out(void)
{
	al_start_timer(go_out_timer);
}

void wait_go_out(Ghost* ghost)
{
	if (al_get_timer_count(go_out_timer) == 3)
	{
		ghost->status = BLOCKED;
		al_stop_timer(go_out_timer);
		al_set_timer_count(go_out_timer, 0);
