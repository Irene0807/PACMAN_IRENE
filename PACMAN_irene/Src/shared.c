...
ALLEGRO_SAMPLE* inMusic = NULL;
ALLEGRO_SAMPLE* inMusic1 = NULL;
ALLEGRO_SAMPLE* inMusic2 = NULL;
ALLEGRO_SAMPLE* PACMAN_MOVESOUND = NULL;
ALLEGRO_SAMPLE* PACMAN_EATSOUND = NULL;
...
void shared_init(void) {
	...
	inMusic = load_audio("Assets/Music/cometome.ogg");
	inMusic1 = load_audio("Assets/Music/cometome.ogg");
	inMusic2 = load_audio("Assets/Music/darari.ogg");
	PACMAN_MOVESOUND = load_audio("Assets/Music/pacman-chomp.ogg");
	PACMAN_EATSOUND = load_audio("Assets/Music/pacman_eatfruit.ogg");
...
void shared_destroy(void) {
	...
	al_destroy_sample(inMusic);
	al_destroy_sample(inMusic1);
	al_destroy_sample(inMusic2);
	al_destroy_sample(PACMAN_MOVESOUND);
	al_destroy_sample(PACMAN_EATSOUND);
