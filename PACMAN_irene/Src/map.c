...
static void draw_speedup(Map* M, const int row, const int col);
static void draw_penetrate(Map* M, const int row, const int col);
...
/* ----------------------------- start of default map code*/
	...
	if (filepath == NULL) {
		...
		if(fscanf(pFile,"%d %d",&M->row_num, &M->col_num) != 2) {//fscanf 回傳值為 傳成功的數值個數
...
void delete_map(Map* M) {
	...
	if(M->map){
		for (int i = 0; i < M->row_num; i++) {
			free(M->map[i]);
			if(!M->map[i]){
				game_abort(stderr, "map char array free error\n");
				return NULL;
			}
		}
		free(M->map);
		...
		for (int col = 0; col < M->col_num; col++) {
			switch (M->map[row][col])
			{
				...
				case '.':
					...
					break;
				case 'S':
					draw_speedup(M, row, col);
					break;
				case 'E':
					draw_penetrate(M, row, col);
					break;
				default:
					break;
			}
...
static void draw_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width/6.0,  al_map_rgb(230, 230, 250));
}
...
static void draw_speedup(Map* M, const int row, const int col) {
	al_draw_scaled_bitmap(speedup, 0, 0, 2500, 1000, map_offset_x + col * block_width + block_width / 2.0-20, map_offset_y + row * block_height + block_height / 2.0-20, 357, 143, 0);
}
static void draw_penetrate(Map* M, const int row, const int col) {
	al_draw_scaled_bitmap(penetrate, 0, 0, 2500, 1000, map_offset_x + col * block_width + block_width / 2.0-15, map_offset_y + row * block_height + block_height / 2.0-15, 330, 120, 0);
}
