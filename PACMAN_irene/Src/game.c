...
ALLEGRO_TIMER* flee_timer;
ALLEGRO_TIMER* go_out_timer;
ALLEGRO_TIMER* speedup_timer;
ALLEGRO_TIMER* penetrate_timer;
ALLEGRO_BITMAP* speedup;
ALLEGRO_BITMAP* penetrate;
...
static void allegro5_init(void) {
	...
	flee_timer = al_create_timer(1.0f);
	go_out_timer = al_create_timer(1.0f);
	speedup_timer = al_create_timer(1.0f);
	penetrate_timer = al_create_timer(1.0f);

	speedup = al_load_bitmap("Assets/laptop.png");
	penetrate = al_load_bitmap("Assets/code.png");
...
static void game_destroy(void) {
	...
	al_destroy_timer(go_out_timer);
	al_destroy_timer(speedup_timer);
	al_destroy_timer(penetrate_timer);
	al_destroy_display(game_display);
	al_destroy_bitmap(speedup);
	al_destroy_bitmap(penetrate);
	free(mouse_state);
}
