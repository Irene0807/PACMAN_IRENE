	...
	*/
	...
	if (game_over) {
		...
		*/
		for (int i = 0; i < 12; i++)
		{
			if (al_get_timer_count(pman->death_anim_counter) / 8 == i)
			{
				al_draw_scaled_bitmap(pman->die_sprite, 0 + 16 * i, 0, 16, 16,
					drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
					draw_region, draw_region, 0
				);
			}
		...
		//game_log("%d",pman->objData.moveCD);
			...
			{
			case LEFT:
				//if (isspeedup && pman->objData.moveCD /3 %2 == 0) movex = 32;
				if (pman->objData.moveCD >= 32) movex = 32;
				else movex = 48;
				break;
			case RIGHT:
				if (pman->objData.moveCD >= 32) movex = 0;
				else movex = 16;
				break;
			case UP:
				if (pman->objData.moveCD >= 32) movex = 64;
				else movex = 80;
				break;
			case DOWN:
				if (pman->objData.moveCD >= 32) movex = 96;
				else movex = 112;
				break;
			default:
				break;
			}
			al_draw_scaled_bitmap(pman->move_sprite, movex, 0, 16, 16,
				drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
				draw_region, draw_region, 0
			);
