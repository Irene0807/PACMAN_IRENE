	...
	if (ghost->status == FLEE) {
		if (!near_powerdown) {
			...
			if (GAME_TICK / 32 == 0)
				...
				movex = 16;
			al_draw_scaled_bitmap(ghost->flee_sprite, movex, 0, 16, 16,
				drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
				draw_region, draw_region, 0
			);
		}
		...
		al_draw_scaled_bitmap(ghost->dead_sprite, movex, 0, 16, 16,
			drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
			draw_region, draw_region, 0
		);
	}
		...
		{
		case LEFT:
			//if (pman->objData.moveCD /3 %2 == 0) movex = 32; 快速開合版
			if (ghost->objData.moveCD >= 32) movex = 32;
			else movex = 48;
			break;
		case RIGHT:
			if (ghost->objData.moveCD >= 32) movex = 0;
			else movex = 16;
			break;
		case UP:
			if (ghost->objData.moveCD >= 32) movex = 64;
			else movex = 80;
			break;
		case DOWN:
			if (ghost->objData.moveCD >= 32) movex = 96;
			else movex = 112;
			break;
		default:
			break;
		}
		al_draw_scaled_bitmap(ghost->move_sprite, movex, 0, 16, 16,
			drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
			draw_region, draw_region, 0
		);
	}
...
bool ghost_movable(Ghost* ghost, Map* M, Directions targetDirec, bool room) {
	...
	case RIGHT:
		...
		break;
	default:
		// for none UP, DOWN, LEFT, RI	GHT direction u should return false.
		return false;
	}
...
void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
	...
	for (Directions i = 1; i <= 4; i++)
		if (ghost_movable(ghost, M, i, 1) && i!= premove && i!= shortestDirection) 	posmove[cnt++] = i;
	if(cnt != 0)
		ghost_NextMove(ghost, posmove[rand()%cnt]);
	else {
		if(al_get_timer_count(power_up_timer) < 1)
